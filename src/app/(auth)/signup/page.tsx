  ...
  return (
   <div className='w-full h-full flex justify-between bg-dark p-2'>
    ...
    <div className='w-full p-6'>
        ...
        <h2 className="text-xl font-bold text-gray-800 dark:text-white">ConnectApp</h2>
    </div>
            <div className="text-md">
                <span className="text-gray-400 dark:text-gray-200 ">Already have an account?</span>
                <a className="font-bold text-primary px-1" href="/sigin">Sign In</a>
            </div>
        </div>
        <div className='flex flex-col gap-6 px-12 mt-15'>
            <h1 className='text-4xl font-bold text-white'>Create Your Account</h1>
            <h1 className='text-xl text-gray-600'>Get started by creating a new account.</h1>
        </div>
                <form noValidate className='px-12 mb-3'>
                     ...
                     <INPUT
                         value={middleName}
                         onChange={(e)=>{ setMiddleName(e.target.value); setErrors(prev=>({...prev, middleName: ''})); }}
                         type="text"
                         placeholder=""
                         label='Middle Name'
                         className="mb-10 text-white "
                     ...
                     <div className="mt-4">
                         <button
                             type="submit"
                             className={`px-4 py-3 w-full rounded bg-primary text-white ${loading ? 'opacity-60 cursor-wait' : ''}`}
                             disabled={loading}
                         >
                ...
                <div className='flex justify-evenly items-center text-gray-200 mt-5'>
                    {/* <button className="px-10 cursor-pointer py-2 flex gap-3 border border-red-500 rounded" onClick={()=>signIn("google")}>
                        ...
                        Google</button> */}
                        <SiginButton img="/b.png" provider="google" providerId="Google" callbackUrl='/' className="border border-red-500"/>
                    <button className="px-10 cursor-pointer py-2 gap-3 border border-primary rounded" onClick={()=>signOut()}>
                        <Image src="/github.png" alt="GitHub" width={20} height={20} className="inline-block mr-2 mb-1"/>
                        GitHub</button>
                        <SiginButton img='/github.png' provider="github" providerId="GitHub" callbackUrl='/' className="border border-primary"/>
                </div>
    </div>
   </div>
  );
